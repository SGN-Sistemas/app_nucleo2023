...
export default function EditResponsaveis({ route, navigation }) {
  ...
  const { nome, cpf, email, telefone1, idade, imagem } =
    route.params.data.Dados;
  ...
  const { atualizaResp, setAtualizaResp, codigoUsuario } =
  ...
  useEffect(() => {
    fetch(url + "RespFilt", {
      ...
      body: JSON.stringify({
        ...
        cod_resp: "" + route.params.data.Dados.id,
      }),
    })
      .then((resp) => resp.json())
      .then((json) => {
        ...
        setImagePerfil(urlImage + "" + json[0].foto);
      });
  }, []);
    ...
    fetch(link, {
      ...
      .then((data) => {
        if (data.INSERIDO) {
          ...
          alert(
            "Não foi possivel inserir a imagem, tente novamente mais tarde!"
          );
        }
        //setLoading(false)
      })
